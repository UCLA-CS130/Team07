...
namespace http {
	namespace server { 
		...
		void connection::start() {
			...
			{
				std::cerr << "Error reading request: " << e << std::endl;
				throw(e);
			}
		}
		  ...
		  : io_service_(), acceptor_(io_service_), socket_(io_service_), context_(io_service_, boost::asio::ssl::context::sslv23)
		{
			...
			{
				std::cerr << "Port number specified is out of possible range, using port 80" << std::endl;
			}
			...
			try {
				if(!config->IsHttps())
				{
					...
					[this](boost::system::error_code ec)
					{
						...
						{
							std::cerr << "Error had occured while accepting connection: "<< ec << std::endl;
							throw(ec);
						}
				...
				std::cerr << "Error had occured while accepting connection: "<< e << std::endl;
				throw(e);
			}
		}
